...
test("site renders real bilingual articles with resolvable static links and filters", async () => {
  ...
  try {
    ...
    assert.equal(doc.querySelectorAll(".catalog .case-resources:not(.inline-resources)").length, 0);
    for (const link of doc.querySelectorAll(".catalog .resource-link")) {
      assert.ok(link.closest(".entry-heading"));
      assert.ok(link.getAttribute("aria-label"));
      assert.ok(link.getAttribute("title"));
    }
