...
> 在演示游戏中完成开局、合法操作、结束和重新开局。记录结束后是否仍能提交普通操作，并核对重开是否清空上一局状态。

从入口卡片进入一款游戏后，先完成最短的一局。结束时尝试重复触发上一步操作，再重新开局。按钮隐藏只是界面表现；规则层也应该拒绝结束态动作。重新开局则应建立新状态，不能继承上局的分数或行动队列。

### 读懂结果，再试一次反例

第一轮跑通后，只改变一个条件：**用户连续点击出牌**。此时应当看到的行为是：同一回合只接受一次有效动作。 将前后两次结果并排保留，核对是判断发生了合理变化，还是某一步没有输出。
