...
> 启动一个测试报告任务，记录任务标识。关闭页面后重新进入原任务，核对历史、运行状态和产物，不重复提交同一个请求。

任务进入运行态后再断开浏览器，避免把请求尚未成功提交误判成远程续跑。重新进入时按原标识查询；若任务仍运行，显示当前状态即可。只有明确失败且支持重试的任务才进入重试流程，页面断线本身不构成重跑理由。

### 读懂结果，再试一次反例

第一轮跑通后，只改变一个条件：**关闭浏览器**。此时应当看到的行为是：任务不被误标取消，重连能查询状态。 将前后两次结果并排保留，核对是判断发生了合理变化，还是某一步没有输出。
