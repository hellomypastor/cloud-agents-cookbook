...
> 分析两个周期的新用户转化率变化。先确认新用户、转化事件和分母定义，再给出查询结果。报告分为观察事实、待验证解释、下一步查询，并附计算依据。

先核对两个周期是否使用相同的新用户定义和观察窗口。如果一个周期尚未结束，即使公式相同也不能直接比较。样例中的下降五个百分点是观察事实；要继续解释原因，应追加渠道或用户分群数据，而不是要求模型把报告写得更肯定。

### 读懂结果，再试一次反例

第一轮跑通后，只改变一个条件：**用户口径与默认口径冲突**。此时应当看到的行为是：确认定义后再执行查询。 将前后两次结果并排保留，核对是判断发生了合理变化，还是某一步没有输出。
