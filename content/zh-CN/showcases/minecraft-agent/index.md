...
> 在测试世界中接近指定目标，到达约定距离后停止。每次动作后重新观察位置；目标不可达或玩家要求停止时返回当前状态。

先在无遮挡区域检查接近和停止，再在路径上增加障碍。观察重点是动作后的真实位置，而不是模型是否说了“已到达”。到达阈值之内应停止继续移动；目标不可达时，应保留失败状态和当前位置，便于玩家重新指定目标。

### 读懂结果，再试一次反例

第一轮跑通后，只改变一个条件：**目标玩家突然移动**。此时应当看到的行为是：根据新位置更新计划。 将前后两次结果并排保留，核对是判断发生了合理变化，还是某一步没有输出。
