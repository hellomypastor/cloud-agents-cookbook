...
以下为演练：当前组件规则使用常规页面间距，活动页设计师希望更紧凑。Agent 先按当前规则指出差异，设计师保留了紧凑布局，并说明它只适用于这次活动。

这条反馈有两种完全不同的解释：全局规则错了，或活动场景需要局部例外。现有证据只支持后者。知识提案应描述“为活动场景补充例外”，并保留常规页面规则。

一份可审查的提案至少包含：原规则版本、触发任务、建议与最终方案的差异、适用项目、支持理由、可能受影响的页面，以及谁负责审核。只保存“用户不喜欢间距”无法支持规则变更。
