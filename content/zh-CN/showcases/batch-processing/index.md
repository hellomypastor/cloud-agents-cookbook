...
以下是合成演练，展示原资料中单位换算场景的接入细节。

| 来源 | 原始字段 | 原始值 | 商品主数据 | 标准结果 |
|---|---|---|---|---|
| 经销商 A | 货号、箱数 | SKU-A，3 箱 | 每箱 12 件 | SKU-A，36 件 |
| 经销商 B | 商品编码、件数 | SKU-A，8 件 | 件为标准单位 | SKU-A，8 件 |
| 经销商 C | 货号、箱数 | SKU-B，2 箱 | 包装规格缺失 | 待确认，不猜数量 |

Agent 可以提出“货号对应商品编码”的映射，但 3 乘 12 应由确定性计算完成。包装规格还需要适用日期或版本：如果历史订单采用旧包装，用今天的规格重算会得到形式正确、业务错误的结果。

结果不是只有 44 件的汇总，还要包含 SKU-B 的待确认记录。如果下游必须收到完整数据，应阻止整组入库；如果允许部分完成，则需要把成功和待确认项分别标记。这个策略应在输入输出约定中写清楚。
...
![批处理成功与失败清单示意](./assets/result-preview.png)

图为合成结果示意。除了成功项，验收时还要检查失败项能否单独重试、回写失败是否触发重复计算，以及规则更新是否保留版本。
