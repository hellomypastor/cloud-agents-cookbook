...
> 查看演示驾驶舱的装置负荷与物料流，选择一个装置追踪其上下游。解释图中单位与数据模式，不将模拟读数描述为生产实测。

先从总览找到一条完整物料路径，再核对节点负荷和连线流量是否使用不同单位。负荷百分比不等于物料吞吐量，不能直接相加。截图用于说明信息如何组织；切换详情或导出报告时，也应保留模拟数据身份。

### 读懂结果，再试一次反例

第一轮跑通后，只改变一个条件：**数据停止更新**。此时应当看到的行为是：显示过期时间，不继续标为实时。 将前后两次结果并排保留，核对是判断发生了合理变化，还是某一步没有输出。
