...
> 检查测试资源的使用情况，列资源、指标时间窗、依赖与证据缺口。指标缺失时保持待判断，只输出建议草稿。

先选一项指标完整的资源，再选一项指标缺失的资源，比较报告是否把后者误当成零使用率。闲置判断还需要时间窗、周期性业务和依赖关系。即使得到低利用率结论，资源调整也应是独立动作，不能作为查询报告的隐含副作用。

### 读懂结果，再试一次反例

第一轮跑通后，只改变一个条件：**指标缺失**。此时应当看到的行为是：标记无法判断，不将其当作零使用率。 将前后两次结果并排保留，核对是判断发生了合理变化，还是某一步没有输出。
