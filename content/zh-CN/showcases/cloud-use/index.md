...
以三个合成资源为例，报告的价值来自区分状态，而不是凑出三条优化建议。

| 资源 | 当前证据 | 还不能确定什么 | 合理输出 |
|---|---|---|---|
| demo-a | 指定时间窗内利用率较低 | 是否承担周期任务 | 候选项，补查调度与业务用途 |
| demo-b | 资源存在，但指标不可用 | 实际负载 | 数据不足，先修复查询链路 |
| demo-c | 利用率波动，已知有定时作业 | 高峰窗口容量需求 | 按业务窗口继续分析 |

这里没有足够信息计算预计节省金额。需要实际计费方式、规格、折扣和变更方案才能估算。将利用率乘以账单金额，不能直接得到可实现的节省。
...
![云资源检查的结果示意](./assets/result-preview.png)

图为合成结果示意，突出“指标缺失”和“尚不能确定”。它不代表任何实际账号的资源状况。
