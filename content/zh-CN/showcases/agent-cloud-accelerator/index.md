...
![云端任务回执](./assets/result-preview.png)

根据本文案例绘制的结果示意，使用合成数据，非产品截图。结果返回原会话，产物保留独立引用
...
> 读取测试文件并生成报告，将产物归档后通知原会话。恢复时先核对检查点和产物，不重复执行已经完成的计算；通知失败单独重试。

把中断点放在报告写入之后、通知发出之前。恢复进程应先确认报告可读且对应同一输入版本，再只重试通知。若仅凭文件存在就跳过计算，可能误用旧文件；若无条件重跑，则可能产生重复产物和重复通知。

### 读懂结果，再试一次反例

第一轮跑通后，只改变一个条件：**计算后归档前中断**。此时应当看到的行为是：恢复后检查产物存在性与完整性。 将前后两次结果并排保留，核对是判断发生了合理变化，还是某一步没有输出。
