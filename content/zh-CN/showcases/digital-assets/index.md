...
![知识检索结果](./assets/result-preview.png)

根据本文案例绘制的结果示意，使用合成数据，非产品截图。保留旧版本追溯，回答引用当前有效版本
...
> 根据当前有效的接口说明回答问题，给出文档版本和引用位置。若命中的是旧版本，请继续查找替代版本；没有有效材料时说明缺口。

准备同一接口的两个版本，让旧版保留容易命中的关键词。检查检索排序是否仍会优先返回旧文档，以及回答是否明确引用 v2。再撤掉当前版本的访问权限：系统应表现为当前材料不可用，而不是为了完成回答退回用户无权访问的材料。

### 读懂结果，再试一次反例

第一轮跑通后，只改变一个条件：**用户无权读取某资产**。此时应当看到的行为是：检索结果和答案均不包含该材料。 将前后两次结果并排保留，核对是判断发生了合理变化，还是某一步没有输出。
