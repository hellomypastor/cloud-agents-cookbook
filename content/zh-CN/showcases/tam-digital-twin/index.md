...
> 诊断测试连接超时。分别列已观察症状、已执行检查、缺少证据和下一步检查，暂不把任何猜测写成确定根因。

先提供网络超时结果，但不提供配置检查。检查报告是否能明确停在“观察到超时”，并提出下一项能够区分候选原因的检查。随后补充配置证据，观察结论是否随新信息更新，而不是固守第一轮猜测。

### 读懂结果，再试一次反例

第一轮跑通后，只改变一个条件：**一个专项工具失败**。此时应当看到的行为是：保留已获得证据，明确缺失检查。 将前后两次结果并排保留，核对是判断发生了合理变化，还是某一步没有输出。
