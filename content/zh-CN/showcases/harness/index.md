...
![任务图交付视图](./assets/result-preview.png)

根据本文案例绘制的结果示意，使用合成数据，非产品截图。验证失败时回退相关节点，保留已完成阶段的证据
...
> 为输入校验功能建立需求、实现、验证三个节点。每个节点记录依据版本和完成证据；需求变化时标记受影响产物，测试通过不自动发布。

先用需求 v1 完成一次实现与验证，然后把允许输入的范围改成 v2。故意保留旧的通过状态，检查图是否将旧实现和对应验证标为过期。此时重新运行旧测试仍可能通过，但它无法证明新需求已满足；需要先更新实现及验收用例。

### 读懂结果，再试一次反例

第一轮跑通后，只改变一个条件：**需求在开发后变更**。此时应当看到的行为是：使相关下游节点失效并重新验证。 将前后两次结果并排保留，核对是判断发生了合理变化，还是某一步没有输出。
