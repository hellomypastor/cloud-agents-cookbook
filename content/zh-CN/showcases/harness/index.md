...
实现基于需求 v1，测试也全部通过。随后用户将范围改成 1 到 50，需求成为 v2。此时旧实现和旧验证都应标记为过期：再跑一遍只检查 1 与 100 的旧用例，即使结果“通过”，也不能证明新需求满足。

失效范围应沿依赖传播；不依赖这项输入的独立工作可以保留。这样既避免整图重跑，也避免旧结果被错误复用。
...
|---|---|---|
| 需求 | v1 改为 v2 | 确认新范围 |
| 实现 | 仍基于 v1 | 标记过期并重做相关改动 |
| 验证 | 旧测试通过 | 补新边界后测试新代码 |
| 发布 | 尚无 v2 验证证据 | 不能沿用旧许可 |
