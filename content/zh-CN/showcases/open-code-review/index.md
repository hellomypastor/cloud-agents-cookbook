...
> 评审指定基准到目标提交的变更。每个发现给出位置、触发输入、实际行为、影响和证据。单独报告未检查文件；没有发现不等同于全量验证通过。

先选择一个包含已知边界缺陷的小变更，同时保留一个正常文件。查看报告能否复现缺陷而不对正常文件产生泛化风格意见。再让一个文件无法读取，检查覆盖报告是否随之变化：发现列表为空和评审完成是两种不同状态。

### 读懂结果，再试一次反例

第一轮跑通后，只改变一个条件：**一个文件超过上下文限制**。此时应当看到的行为是：明确未覆盖范围，不报告全量完成。 将前后两次结果并排保留，核对是判断发生了合理变化，还是某一步没有输出。
