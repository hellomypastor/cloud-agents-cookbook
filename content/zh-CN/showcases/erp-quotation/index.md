...
> 为十套测试备件生成报价草稿。先列物料匹配、数量、单价来源和小计。税费、运费或价格有效期缺失时列为待确认项，不自行补值，不发送正式报价。

先让使用者确认“套”包含哪些物料，再取价格。如果增强配置替换了物料，必须重新计算对应行，不能只在原总价上加一个模型估计的比例。样例的 1,200 只是未税小计；缺少税费和运费时，草稿可以交付供讨论，正式总价仍应留空。

### 读懂结果，再试一次反例

第一轮跑通后，只改变一个条件：**物料名称有多个匹配**。此时应当看到的行为是：列出差异并补问，不任选料号。 将前后两次结果并排保留，核对是判断发生了合理变化，还是某一步没有输出。
