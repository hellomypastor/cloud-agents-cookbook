...
在测试官网发起两个独立会话，验证流式回答、连接恢复和错误提示，特别检查一名访客的内容不会进入另一名访客的对话。
...
在服务端设置凭证，浏览器只调用站内接口；使用不含真实客户资料的知识样本。

#### 2. 创建独立会话

为两名访客建立不同会话，分别记录消息与事件流归属。

#### 3. 模拟连接恢复

断开一次流连接后重连，核对增量事件没有重复拼接。

#### 4. 核对最终结果

最终消息到达后对齐完整回答，明确超时与服务端错误。
