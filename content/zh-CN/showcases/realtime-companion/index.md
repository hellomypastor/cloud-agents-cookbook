...
> 进行一轮简短语音交流。我插话时立即停止旧回复，优先处理新的话语。结束交流后关闭本轮输入与播放。

在第一轮回复播放中开始第二次提问。分别检查声音是否停止、旧音频队列是否清空、字幕是否串入新一轮。仅取消模型生成不足以停止已经排入播放器的音频；需要将生成、播放和字幕都关联到同一个轮次。

### 读懂结果，再试一次反例

第一轮跑通后，只改变一个条件：**回应期间用户插话**。此时应当看到的行为是：旧回应停止，新轮次不混入旧内容。 将前后两次结果并排保留，核对是判断发生了合理变化，还是某一步没有输出。
